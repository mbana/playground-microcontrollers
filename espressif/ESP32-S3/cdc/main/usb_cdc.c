...
static const char *TAG = "usb_cdc.c";

#define EXAMPLE_USB_HOST_PRIORITY (20)
#define EXAMPLE_TX_STRING ("logread -f\n")
#define EXAMPLE_TX_TIMEOUT_MS (1000)
#define MAX_CDC_DEVICES (5)
#define ESPRESSIF_VID \
	(0x303A) // 0x303A Espressif VID, used in TinyUSB devices or in
// USB-Serial-JTAG devices
#define APP_QUIT_PIN CONFIG_APP_QUIT_PIN

/** Open CDC device handles; NULL means slot is free. Needed for APP_QUIT (close
 * all) and to clear slot on disconnect. */
...
typedef struct {
	enum {
		APP_QUIT, /**< Request to exit: uninstall CDC driver and USB
			     host lib */
			 ...
			 void *user_ctx)
{
	...
	default:
		ESP_LOGW(TAG,
			 "Unsupported CDC event: %d (possibly suspend/resume)",
			 event->type);
		break;
	}
}
...
static void new_dev_cb(usb_device_handle_t usb_dev)
{
	...
	ESP_LOGI(TAG, "New CDC device connected VID=0x%04X PID=0x%04X", vid,
		 pid);
...
/**
 * @brief Run CDC demo (print descriptor, tx, control line, line coding) for one
 * device.
 */
static void run_cdc_demo(int slot)
{
	...
	ESP_ERROR_CHECK(cdc_acm_host_data_tx_blocking(
		cdc_dev, (const uint8_t *)EXAMPLE_TX_STRING,
	...
	if (err == ESP_ERR_NOT_SUPPORTED) {
		...
		ESP_LOGI(TAG,
			 "\t- Control line state set to DTR=false, RTS=true");
		vTaskDelay(pdMS_TO_TICKS(20));
		cdc_acm_host_set_control_line_state(cdc_dev, false, false);
	}
	...
	if (err == ESP_ERR_NOT_SUPPORTED) {
		...
		ESP_LOGI(TAG,
			 "\t- Line Coding Get: Rate: %" PRIu32
			 ", Stop bits: %" PRIu8 ", Parity: %" PRIu8
			 ", Databits: %" PRIu8,
	...
	ESP_LOGI(TAG,
		 "Example finished for device in slot %d. You can disconnect "
		 "or connect another device.",
		 slot);
}
...
static void usb_lib_task(void *arg)
{
	...
	xTaskNotifyGive(
		arg); // Notify the main task that the USB host is installed

	bool has_clients =
		true; // We installed CDC-ACM driver, so we have clients
	while (1) {
...
void app_main(void)
{
	...
	ESP_LOGI("app_main",
		 "Waiting for CDC devices. Press Boot button to quit.");
	...
	while (running) {
		...
		case APP_DEVICE_CONNECTED: {
			...
			if (slot < 0) {
				ESP_LOGW(
					"app_main",
					"No free slots for new CDC device (max %d)",
					MAX_CDC_DEVICES);
				continue;
			}
			...
			cdc_acm_dev_hdl_t cdc_dev = example_cdc_open(
				msg.data.new_dev.vid, msg.data.new_dev.pid,
				&dev_config);
			if (cdc_dev == NULL) {
				continue;
			}
		...
		case APP_DEVICE_DISCONNECTED: {
			ESP_LOGI("app_main", "Device disconnected from slot %d",
				 msg.data.device_slot);
