...
//     break;
//   default:
//     ESP_LOGW(TAG, "Unsupported CDC event: %d (possibly suspend/resume)",
//              event->type);
//     break;
//   }
// }
...
//   ESP_LOGI(TAG, "Opening CDC device VID=0x%04X PID=0x%04X - needle=0x%04X", vid, pid, 0x04d9);
...
//       cdc_dev, (const uint8_t *)EXAMPLE_TX_STRING, strlen(EXAMPLE_TX_STRING),
...
//   } else {
//     ESP_LOGI(TAG,
//              "\t- Line Coding Get: Rate: %" PRIu32 ", Stop bits: %" PRIu8
...
//       if (slot < 0) {
//         ESP_LOGW("app_main", "No free slots for new CDC device (max %d)",
//                  MAX_CDC_DEVICES);
//         continue;
//       }
